...
void showMenu(){
	...
	printf(BOLD "Criar diretório\n" );

	printf(BOLD YELLOW "criaa     ->   ") ;
	printf(BOLD "Criar arquivo\n" );


	printf(BOLD MAGENTA "removed   ->   ") ;
	printf(BOLD "Remover diretório\n");

	printf(BOLD MAGENTA "removea   ->   ") ;
	printf(BOLD "Remover arquivo\n");


	printf(BOLD GREEN "verd      ->   ") ;
	printf(BOLD "Mostrar arquivos e diretórios \n");

	printf(BOLD GREEN "verset    ->   ") ;
	printf(BOLD "Mostrar setores de um arquivo\n");

	printf(BOLD GREEN "map       ->   ") ;
	printf(BOLD "Mostrar o mapa de setores\n" );

	printf(BOLD GREEN "tree      ->   ") ;
	printf(BOLD "Mostrar a arvore de arquivos\n");

	printf(BOLD CYAN "help      ->   ") ;
	printf(BOLD "Exibir menu de ajuda\n");

	printf(BOLD RED "exit      ->   ") ;
	printf(BOLD "Fechar programa\n\n" RESET);
...
void create_directory(char *path) {
	...
	strftime (new_directory->create_date, sizeof (new_directory->create_date), "%d/%m/%Y %H:%M:%S",ptm);
...
void create_file(char *path, float size) {
	...
	strftime (new_file->create_date, sizeof (new_file->create_date), "%d/%m/%Y %H:%M:%S",ptm);
	for (int k = 0; k < NUM_SECTORS; k++) {
...
int showDataInfo(char *path){
	...
	count = walkthroughTree(parent, 0);
	printf("\nConteudo: %d itens, totalizando %d bytes\n", count, tam);
	count = 0;
	for (int i = 0; i < NUM_SECTORS; i++) {
		if (bitmap[i] == 1){
			count++;
	...
	printf("Espaço ocupado em disco: %d bytes\n", count);
	printf("Espaço livre em disco: %d bytes\n", (TotalSize-count));
	free(root);
	free(path_sub);
	return 1; // Retorna sem deletar o diretório
	...
	while(1){
		...
		if (strcmp(args[0], "criad") == 0){
			if(args[1] != NULL)
				...
				printf(BOLD RED"Erro de Sintaxe: Argumentos insuficientes\n" RESET);
			...
			printf(BOLD "Fechando o simulador...\n" RESET);
			return EXIT_SUCCESS;			
		}else if(strcmp(args[0], "help") == 0){
			showMenu();
		}
		else{
			printf(BOLD"Comando não encontrado...\n");
			printf(BOLD "Para mais informações sobre os comandos, digite 'help'\n" RESET);
		}
	}
	return 0;
}
